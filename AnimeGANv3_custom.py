...
class AnimeGANv3(object) :
    ...
    def load_weights_onnx(self, filename_onnx) :
        # -------------------------------------------------------------------------
    ...
    def replace_weights_generator(self, filename_onnx) :
        ...
        # -------------------------------------------------------------------------
        onnx_weights = self.load_weights_onnx(filename_onnx)
    ...
    def build_train(self):
        ...
        if WB :
            self.sty_loss = 0.5 * (self.s22  + self.s33 +  self.s44)
            self.color_loss = bw_loss(self.generated)
            self.white_loss = white_region_loss(self.anime_sty_gray, self.fake_sty_gray, threshold=0.85)
        else :
        ...
        if WB :
            self.G_support_loss = self.g_adv_loss + self.con_loss + self.sty_loss + self.rs_loss +  self.color_loss + self.tv_loss + self.white_loss
        else :
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch+1):
            ...
            if (epoch + 1) >= self.init_G_epoch:
                # Result Image
                val_files = glob(f"./dataset/val/*.*")
                save_path = f"./{self.sample_dir}/{epoch:03d}/"
                check_folder(save_path)
                for i, sample_file in enumerate(val_files):
                    print('val: ', sample_file, "->", save_path )
                    ...
                    save_images(val_real, save_path+f"{i:03d}_a.jpg")
                    save_images(test_s1, save_path+f"{i:03d}_b.jpg")
                    save_images(test_s0, save_path+f"{i:03d}_c.jpg")
                    save_images(test_m, save_path+f"{i:03d}_d.jpg")
            """
