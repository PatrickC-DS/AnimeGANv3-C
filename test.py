...
def test(checkpoint_dir, save_dir, test_dir, onnx=""):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, gpu_options=gpu_options)) as sess:
        ...
        for i, sample_file  in tqdm(list(enumerate(test_files))):
            ...
            print(f"\n--- Diagnostic Image {i} ---")
            print(f"Entrée  (test_real) : Min={real.min():.4f}, Max={real.max():.4f}")
            print(f"Branche s1 (sortie) : Min={s1.min():.4f}, Max={s1.max():.4f}, Mean={s1.mean():.4f}")
            print(f"Branche s0 (brute)  : Min={s0.min():.4f}, Max={s0.max():.4f}, Mean={s0.mean():.4f}")
            print(f"Branche m  (main)   : Min={m.min():.4f}, Max={m.max():.4f}, Mean={m.mean():.4f}")
